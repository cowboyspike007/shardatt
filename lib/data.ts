export const introText =
  "Sharda T.T. College is committed to preparing competent, compassionate and professionally responsible teachers through a balanced emphasis on academic study, practical training and value-based education.";

export const address =
  "N.H.-11, Udaipuria Mode, Lohrwada Road, Chomu, Jaipur, Rajasthan 303702 India";

export const courses = [
  {
    name: "B.Ed",
    duration: "2 years",
    intake: "100",
    description:
      "Bachelor of Education is a professional course after graduation for students preparing to work as school teachers.",
    fees: [
      ["First Year", "27000/-"],
      ["Second Year", "26880/-"]
    ]
  },
  {
    name: "B.A.-B.Ed",
    duration: "4 years",
    intake: "50",
    description:
      "An integrated arts and education program for learners who want to begin teacher preparation after class 12.",
    fees: [
      ["First Year", "27000/-"],
      ["Second Year", "26880/-"],
      ["Third Year", "26880/-"],
      ["Fourth Year", "26880/-"]
    ]
  },
  {
    name: "B.Sc.-B.Ed",
    duration: "4 years",
    intake: "50",
    description:
      "An integrated science and education program combining subject knowledge with professional teaching practice.",
    fees: [
      ["First Year", "27000/-"],
      ["Second Year", "26880/-"],
      ["Third Year", "26880/-"],
      ["Fourth Year", "26880/-"]
    ]
  }
];

export const newsLinks = [
  {
    label: "NCTE Registration B.Ed College",
    href: "https://drive.google.com/file/d/1Mr1LDc9Aml47OVvbzEQDwYWlo3zYsvSH/view?usp=sharing"
  },
  {
    label: "Job Form",
    href: "https://forms.office.com/Pages/ResponsePage.aspx?id=DQSIkWdsW0yxEjajBLZtrQAAAAAAAAAAAAMAANVc_l9UNEc3T0EwRE1SV0k1NThVVTBYNlJXRjFLRy4u"
  },
  {
    label: "State NOC B.Ed College",
    href: "https://drive.google.com/file/d/14XGBgfGZYsN48NK6pOoBSCUpqR6GqKXi/view?usp=sharing"
  },
  {
    label: "NCTE Registration B.Sc.-B.Ed",
    href: "https://drive.google.com/file/d/1k51IkG40TX2sUmEz6uLuC2hot-QxF_Jt/view?usp=sharing"
  }
];

export const values = [
  {
    title: "Integrity and Fairness",
    image: "/assets/img/integrity.png",
    text: "Transparent leadership, consistent decision making, and humility help students experience a fair academic environment."
  },
  {
    title: "Equity",
    image: "/assets/img/equality.png",
    text: "The college supports students with resources that fit different circumstances, because learning improves when everyone can participate."
  },
  {
    title: "Creativity and Excellence",
    image: "/assets/img/design-thinking.png",
    text: "Students receive opportunities to polish their skills, practice new ideas, and grow into confident teachers."
  }
];

export const libraryStats = [
  { label: "Total Books", value: "18,960", image: "/assets/img/library.svg" },
  { label: "Journals", value: "15", image: "/assets/img/journal.svg" },
  { label: "Encyclopedia", value: "5", image: "/assets/img/encyclopedia.svg" }
];

export const documents = [
  {
    title: "NCTE Registration Letter",
    links: [
      ["B.Ed", "https://drive.google.com/file/d/1Mr1LDc9Aml47OVvbzEQDwYWlo3zYsvSH/view?usp=sharing"],
      ["B.A.-B.Ed/B.Sc.-B.Ed 4 yr Integrated", "https://drive.google.com/file/d/1k51IkG40TX2sUmEz6uLuC2hot-QxF_Jt/view?usp=sharing"]
    ]
  },
  {
    title: "State NOC",
    links: [
      ["B.Ed State NOC", "https://drive.google.com/file/d/1tROkrAUZsvbAZXmk-GT8b8__gWYkpQwr/view?usp=sharing"],
      ["Integrated State NOC", "https://drive.google.com/file/d/1QwybiGkDovwavqi2-tXfsSuVqe4mjxzm/view?usp=sharing"],
      ["Fire NOC", "https://drive.google.com/file/d/18aGnvbQJnMpSnbgImcQeC24D9K__1guW/view?usp=sharing"]
    ]
  },
  {
    title: "Staff Details",
    links: [
      ["B.Ed", "/documents/bed-staff.pdf"],
      ["B.A.-B.Ed/B.Sc.-B.Ed 4 yr Integrated", "/documents/integrated-staff.pdf"]
    ]
  },
  {
    title: "FDR",
    links: [
      ["B.Ed", "https://drive.google.com/file/d/1aZz7ZFXcjgoAxg2XjuchdO9Iqn3QFhOb/view?usp=sharing"],
      ["B.A.-B.Ed/B.Sc.-B.Ed 4 yr Integrated", "https://drive.google.com/file/d/1fG87HkJA-AEnHlZhkjrJ2Oj1KY1I6wxY/view?usp=sharing"]
    ]
  },
  {
    title: "University Affiliation",
    links: [
      ["B.Ed Affiliation 2024", "https://drive.google.com/file/d/1UornQkSoNW7MEJ0aGZqpcwMZ-YLt5ylU/view?usp=sharing"],
      ["B.Ed Affiliation 2025", "https://drive.google.com/file/d/1W9FOH5mgAsbOl5WRbctj0W9V3s8wLzNG/view?usp=sharing"],
      ["Integrated Affiliation 2024", "https://drive.google.com/file/d/1gu6Gbrh6zlUULlPmutJ8r8g4O83lQrdr/view?usp=sharing"],
      ["Integrated Affiliation 2025", "https://drive.google.com/file/d/1NXXRDMElH1TZM4v-Jbz5ZTBpHHJi1qyM/view?usp=sharing"]
    ]
  },
  {
    title: "Financial Documents",
    links: [
      ["Audit 2023", "https://drive.google.com/file/d/1Jw7HwLvbXIvK1DvCE4k01T62ukW8w-PD/view?usp=sharing"],
      ["Audit 2024", "https://drive.google.com/file/d/1eWYdk5YL37u9osgNeIGnqGN6Dg7lBpTN/view?usp=sharing"],
      ["Audit 2025", "https://drive.google.com/file/d/1FaoAN3yNMHaUnmRQQM_2msYLFYnHPHQE/view?usp=sharing"],
      ["Balance Sheet 2023", "https://drive.google.com/file/d/1M5c4LwZoh4utrzuJyKlxRvP37va3CYxl/view?usp=sharing"],
      ["Balance Sheet 2024", "https://drive.google.com/file/d/1y-mWTUqB2R2nY3_Cp_m7IVgQfHaIJR8T/view?usp=sharing"],
      ["Balance Sheet 2025", "https://drive.google.com/file/d/16ittkhJ9nRTLR4rJiITv331fFh37jBLx/view?usp=sharing"]
    ]
  },
  {
    title: "AISHE",
    links: [["AISHE Certificate", "https://drive.google.com/file/d/1MO2Fwx5H8U_kkF1TK5BbyRZaxljWj7jh/view?usp=sharing"]]
  },
  {
    title: "Land Documents",
    links: [
      ["Land Conversion Letter", "https://drive.google.com/file/d/1KOMhxnXiDu-S4R4VeKbhunzXIsP2XWoF/view?usp=sharing"],
      ["Land Document", "https://drive.google.com/file/d/19xe5wT7D4C6HrYc0ofaxI0amAzdZpjE9/view?usp=sharing"]
    ]
  },
  {
    title: "Infrastructure",
    links: [
      ["Building Completion Certificate", "https://drive.google.com/file/d/1RYg817HmgHROz4NM5wPNfcHg6XqmLCQz/view?usp=sharing"],
      ["Building Safety Certificate", "https://drive.google.com/file/d/1UI4mqEMLLbGBoLT_7NGglWWPLYj9qAQj/view?usp=sharing"],
      ["Blueprint", "https://drive.google.com/file/d/1t0Qc2w5D-qgPux4v6x2mfWonfuXexq1W/view?usp=sharing"]
    ]
  }
];

export const galleryGroups = [
  { title: "Building", images: ["/assets/img/building01.jpg"] },
  {
    title: "Classrooms",
    images: [
      "/assets/img/classroom01.jpg",
      "/assets/img/classroom02.jpg",
      "/assets/img/classroom03.jpg",
      "/assets/img/classroom04.jpg"
    ]
  },
  {
    title: "Labs",
    images: [
      "/assets/img/lab01.jpg",
      "/assets/img/lab02.jpg",
      "/assets/img/lab03.jpg",
      "/assets/img/lab04.jpg",
      "/assets/img/lab05.jpg"
    ]
  },
  { title: "Computer Lab", images: ["/assets/img/computer-lab-2026.jpeg"] },
  { title: "Seminar Hall", images: ["/assets/img/seminarhall.jpg"] },
  { title: "Library", images: ["/assets/img/Library.jpg"] },
  { title: "Playground", images: ["/assets/img/playground01.jpg", "/assets/img/playground02.jpg"] }
];

export const faculty = [
  ["Dr. Brijpal Singh", "Principal", "M.A.-M.Ed., Ph.D.", "Education", "/assets/img/staff/brijpal.jpeg"],
  ["Suresh Kumar", "Lecturer", "M.A.-M.Ed., NET", "Geography", "/assets/img/staff/suresh.jpg"],
  ["Jitendra Kumar", "Lecturer", "M.A.-M.Ed., Ph.D.", "Psychology", "/assets/img/staff/jitendra.jpeg"],
  ["Dr. Narendra Kumar Sharma", "Lecturer", "M.Sc.-M.Ed., Ph.D.", "Chemistry", "/assets/img/staff/narendra.jpeg"],
  ["Raj Laxmi Bunkar", "Lecturer", "M.A.-M.Ed., NET", "Hindi", "/assets/img/staff/rajlaxmi.jpeg"],
  ["Kanishk Pareek", "Lecturer", "M.A., NET", "Art and Craft", "/assets/img/staff/kanishk.jpeg"],
  ["Anshu Samodiya", "Lecturer", "M.P.Ed., NET", "Physical Education", "/assets/img/staff/anshu.jpeg"],
  ["Sunil Kumar Saini", "Lecturer", "M.Mus., NET", "Music", "/assets/img/staff/sunil.jpeg"]
];

export const integratedFaculty = [
  ["Dr. Pushpa Kumari", "Principal", "M.A.-M.Ed., NET, Ph.D.", "Hindi", "/assets/img/staffintegrated/Pushpa.jpg"],
  ["Mr. Chiranji Lal", "Lecturer", "M.A.-M.Ed., NET, Ph.D.", "Political Science", "/assets/img/staffintegrated/Chiranji.jpg"],
  ["Mr. Jai Prakash", "Lecturer", "M.A.-M.Ed., NET", "Pedagogy", "/assets/img/staffintegrated/Jaiprakash.jpeg"],
  ["Pratibha", "Lecturer", "M.Sc.-M.Ed., NET", "Chemistry", "/assets/img/staffintegrated/Pratibha.jpg"],
  ["Dr. Meenakshi", "Lecturer", "M.A.-M.Ed., Ph.D.", "Pedagogy", "/assets/img/staffintegrated/Meenakshi.jpg"],
  ["Dr. Amjad Khan", "Lecturer", "M.P.Ed., Ph.D.", "Physical Education", "/assets/img/staffintegrated/Amjad.jpg"]
];

export const bedStaff = [
  {
    serial: 1,
    name: "Dr. Rajendra Prasad Saini",
    fatherName: "Ram Pratap Saini",
    image: "/staff-images/staff-01.jpg",
    designation: "Principal",
    dob: "07.08.1975",
    qualification: "M.A.-M.Ed., Ph.D.",
    subject: "Biology, Gen. Sci.",
    experience: "13",
    joiningDate: "01.06.2022",
    payScale: "37400-67000"
  },
  {
    serial: 2,
    name: "Saleem Raza",
    fatherName: "Mohammad Aslam",
    image: "/staff-images/staff-02.jpg",
    designation: "Lecturer",
    dob: "01.05.1988",
    qualification: "M.Sc.-M.Ed., NET",
    subject: "Education",
    experience: "8",
    joiningDate: "12.09.2019",
    payScale: "15600-39100"
  },
  {
    serial: 3,
    name: "Mahesh Kumar Sharma",
    fatherName: "Bajrang Lal Sharma",
    image: "/staff-images/staff-03.jpg",
    designation: "Lecturer",
    dob: "05-02-1982",
    qualification: "M.A.-M.Ed., NET",
    subject: "Physics",
    experience: "8",
    joiningDate: "11.09.2019",
    payScale: "15600-39100"
  },
  {
    serial: 4,
    name: "Mr. Ashok Guliya",
    fatherName: "Prahlad Sahay Guliya",
    image: "/staff-images/staff-04.jpg",
    designation: "Lecturer",
    dob: "15.09.1997",
    qualification: "M.Sc.-M.Ed., NET",
    subject: "Geography",
    experience: "2",
    joiningDate: "13.08.2023",
    payScale: "15600-39100"
  },
  {
    serial: 5,
    name: "Pappu Ram Rao",
    fatherName: "Kana Ram Rao",
    image: "/staff-images/staff-05.jpg",
    designation: "Lecturer",
    dob: "25.07.1986",
    qualification: "M.A.-M.Ed., NET",
    subject: "Mathematics",
    experience: "4",
    joiningDate: "10.01.2024",
    payScale: "15600-39100"
  },
  {
    serial: 6,
    name: "Brij Pal Singh",
    fatherName: "Param Singh",
    image: "/staff-images/staff-06.jpg",
    designation: "Lecturer",
    dob: "20-10-1980",
    qualification: "M.Sc.-M.Ed., NET",
    subject: "Commerce / Economics",
    experience: "8",
    joiningDate: "20.12.2021",
    payScale: "15600-39100"
  },
  {
    serial: 7,
    name: "Dr. Ram Singh",
    fatherName: "Sardara Ram",
    image: "/staff-images/staff-07.jpg",
    designation: "Lecturer",
    dob: "28-07-1960",
    qualification: "M.Com.-M.Ed., Ph.D.",
    subject: "Education",
    experience: "20",
    joiningDate: "10.09.2022",
    payScale: "15600-39100"
  },
  {
    serial: 8,
    name: "Sunil Choudhary",
    fatherName: "Gopal Lal Guliya",
    image: "/staff-images/staff-08.jpg",
    designation: "Lecturer",
    dob: "15-10-1988",
    qualification: "M.A.-M.Ed., NET",
    subject: "English",
    experience: "03",
    joiningDate: "03.06.2023",
    payScale: "15600-39100"
  },
  {
    serial: 9,
    name: "Raj Laxmi Bunkar",
    fatherName: "Arjun Lal Bunkar",
    image: "/staff-images/staff-09.jpg",
    designation: "Lecturer",
    dob: "28-05-1985",
    qualification: "M.A.-M.Ed., NET",
    subject: "Hindi",
    experience: "0",
    joiningDate: "15-04-2024",
    payScale: "15600-39100"
  },
  {
    serial: 10,
    name: "Dr. Narendra Kumar Sharma",
    fatherName: "Rambabu Prasad Sharma",
    image: "/staff-images/staff-10.jpg",
    designation: "Lecturer",
    dob: "01.06.1984",
    qualification: "M.Sc.-M.Ed., Ph.D.",
    subject: "Chemistry",
    experience: "10",
    joiningDate: "12.09.2021",
    payScale: "15600-39100"
  },
  {
    serial: 11,
    name: "Ummed Singh Meena",
    fatherName: "Madan Lal Meena",
    image: "/staff-images/staff-11.jpg",
    designation: "Lecturer",
    dob: "08-06-1984",
    qualification: "M.A.-M.Ed., NET",
    subject: "Social Studies",
    experience: "09",
    joiningDate: "20.11.2013",
    payScale: "15600-39100"
  },
  {
    serial: 12,
    name: "Kanishk Pareek",
    fatherName: "Satya Prakash Pareek",
    image: "/staff-images/staff-12.jpg",
    designation: "Lecturer",
    dob: "16.03.1997",
    qualification: "M.A., NET",
    subject: "Art & Craft",
    experience: "3",
    joiningDate: "10.09.2019",
    payScale: "15600-39100"
  },
  {
    serial: 13,
    name: "Vinita Sharma",
    fatherName: "Sita Ram Sharma",
    image: "/staff-images/staff-13.jpg",
    designation: "Lecturer",
    dob: "16-12-1985",
    qualification: "M.A.-M.Ed., SET",
    subject: "History",
    experience: "3",
    joiningDate: "01.05.2024",
    payScale: "15600-39100"
  },
  {
    serial: 14,
    name: "Sanwar Mal",
    fatherName: "Moti Ram",
    image: "/staff-images/staff-14.jpg",
    designation: "Lecturer",
    dob: "01.07.1973",
    qualification: "M.A.-M.Ed., SLET",
    subject: "Psychology",
    experience: "10",
    joiningDate: "12.09.2019",
    payScale: "15600-39100"
  },
  {
    serial: 15,
    name: "Anshu",
    fatherName: "Rakesh",
    image: "/staff-images/staff-15.jpg",
    designation: "Lecturer",
    dob: "23.04.1994",
    qualification: "M.P.Ed., NET",
    subject: "Physical Education",
    experience: "4",
    joiningDate: "06.09.2020",
    payScale: "15600-39100"
  },
  {
    serial: 16,
    name: "Sunil Kumar",
    fatherName: "Kailash Chand",
    image: "/staff-images/staff-16.jpg",
    designation: "Lecturer",
    dob: "18.08.1983",
    qualification: "M.Mus., NET",
    subject: "Music",
    experience: "4",
    joiningDate: "06.09.2020",
    payScale: "15600-39100"
  }
];

export const studentIntake = [
  { program: "B.A.-B.Ed. 4 Year Integrated", count: 39, qualification: "12th" },
  { program: "B.Sc.-B.Ed. 4 Year Integrated", count: 34, qualification: "12th" }
];
